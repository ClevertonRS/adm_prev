<?php
/**
 * Detalhe de uma Preventiva em Revisão (somente-leitura).
 * Mostra a análise e a conclusão do atendimento (tabela atendimentos) e as
 * imagens de evidência agrupadas por momento, com um botão para definir como
 * concluída.
 * Requerido por index.php na rota /revisao/{id}. Variáveis: $pdo, $user, $path.
 */
$base = GPON_BASE_PATH;
preg_match('#^/revisao/(\d+)$#', $path, $mm);
...
<head>
  ...
  <title>Revisão #<?= $previewId ?> — Radar GPON</title>
  ...
  <style>
    ...
    /* Hero — card flutuante com borda azul (identifica a Revisão) */
    .prev-hero {
      background:#ffffff;
      border:1px solid #c7d2fe; border-top:4px solid #1d4ed8;
    ...
    .prev-hero .ph-title i { color:#1d4ed8; }
    ...
    .prev-hero .ph-item { background:#eef2ff; border:1px solid #c7d2fe; border-radius:10px; padding:9px 12px; }
    ...
    /* Ações finais — definir como concluída */
    .prev-card-conclusao-acoes { display:flex; justify-content:flex-end; gap:8px; margin-top:4px; }
    .btn-concluir {
      background:linear-gradient(135deg,#0ea47c,#0b7f6a);
      color:#fff; font-weight:700; letter-spacing:.02em;
      padding:10px 22px; border:none; border-radius:10px; font-size:13px;
      display:inline-flex; align-items:center; gap:8px;
      box-shadow:0 4px 14px rgba(11,127,106,.30);
      transition:transform .15s ease, box-shadow .15s ease, filter .15s ease;
    }
    .btn-concluir:hover { filter:brightness(1.08); transform:translateY(-1px); box-shadow:0 6px 18px rgba(11,127,106,.38); }
    .btn-concluir:active { transform:translateY(0); }
  </style>
</head>
<body>
...
<header class="gpon-topbar">
  <div class="topbar-left">
    ...
    <div class="topbar-logo-icon" onclick="location.href='<?= $base ?>/revisao'" title="Radar GPON" style="cursor:pointer">📡</div>
    <div class="topbar-brand">
      <span class="brand-title">Radar GPON</span>
      <span class="brand-sub">Revisão #<?= $previewId ?></span>
    </div>
  </div>
  <div class="topbar-actions">
    <a href="<?= $base ?>/preventivo#lista-em_revisao" class="tbtn" title="Voltar para Em Revisão">
      <i class="bi bi-arrow-left"></i><span>Em Revisão</span>
    </a>
  ...
  <main class="admin-main">
    <div style="padding:20px; max-width:1000px; margin-left:auto; margin-right:auto">
      ...
      <div id="prev-content" style="display:none">
        ...
        <!-- Hero: dados da preventiva em revisão -->
        <div class="prev-hero">
          <div class="ph-title"><i class="bi bi-clipboard-check-fill"></i> <span id="hdr-gpon" class="mono"></span> <i class="bi bi-arrow-right" style="font-size:15px;opacity:.6"></i> <span id="hdr-splitter" class="mono"></span></div>
        ...
        <div class="prev-card-conclusao-acoes">
          <button type="button" class="btn-concluir" id="btn-concluir">
            <i class="bi bi-check2-circle"></i> Definir como Concluída
          </button>
        </div>
...
<script src="<?= $base ?>/assets/js/preventivo/revisao-detalhe.js?v=<?= filemtime(__DIR__ . '/../../assets/js/preventivo/revisao-detalhe.js') ?>"></script>
<script>
(function () {
  'use strict';
  ...
  // Na página de Revisão os painéis ficam em /preventivo: qualquer item do
