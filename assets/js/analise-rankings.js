...
(function(A) {
  ...
  // ── Preventiva: renderização do botão/badge de Ação e modal de criação
  // extraídos para assets/js/shared/preventiva-modal.js (window.PreventivaModal),
  // reaproveitado também pela coluna "Ação" do Heatmap.
  PreventivaModal.init({
    basePath: window.GPON_BASE_PATH || '',
    onCreated: function() { A.loadAnalise(); },
  });
  ...
  A.initCombTable = function(data) {
    ...
    A.dtComb = $('#tbl-comb').DataTable({
      ...
      columns: [
        ...
        { data: null, orderable: false, width: '190px', render: function(_, __, row) { return PreventivaModal.renderAcaoHtml(row); } },
    ...
    function _combSearch(v) {
      ...
      A.dtComb.order([[7, 'asc']]).clear().rows.add(finalRows).draw();
      var n = scored.length;
  ...
  // Abertura do modal "Preventiva"/"Nova preventiva", o link "Ver preventiva"
  // e o submit de #preventiva-submit são tratados por PreventivaModal.init()
  // (delegação de clique em document, registrada acima) — não precisam de
  // handlers próprios aqui.
