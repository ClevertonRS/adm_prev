(function () {
  ...
  fetch(base + '/api/preventiva/' + id)
    ...
    .catch(function () {
      loading.style.display = 'none';
      alert('Erro ao carregar a revisão.');
    });
  ...
  if (btnConcluir) {
    btnConcluir.addEventListener('click', function () {
      Swal.fire({
        ...
        text: 'A preventiva será marcada como concluída. Deseja continuar?',
        icon: 'warning',
        ...
        fetch(base + '/api/preventiva/' + id + '/concluir', {
          method: 'POST',
          ...
          .then(function (res) {
            if (!res || !res.ok) {
              Swal.fire('Falha', (res && res.message) || 'Não foi possível concluir a preventiva.', 'error');
              return;
            }
            Swal.fire({
              title: 'Preventiva concluída',
              text: 'A preventiva foi definida como concluída.',
              icon: 'success',
          ...
          .catch(function () {
            Swal.fire('Erro', 'Não foi possível concluir a preventiva.', 'error');
          });
      });
    });
  }
